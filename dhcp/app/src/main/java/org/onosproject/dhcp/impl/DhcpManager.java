...
public class DhcpManager implements DhcpService {
     ...
     */
    private Map<Ip4Address, MacAddress> ip4_to_mac = Maps.newConcurrentMap();
    private Map<MacAddress, List<Pair<DeviceId, PortNumber>>> knownLocation = Maps.newConcurrentMap();
    ...
    public Ip4Address getIPAddress(MacAddress mac) {
        ...
        Ip4Address ipOffered = dhcpStore.suggestIP(HostId.hostId(mac), requestedIP);
        if (ipOffered == null) {
            log.info("Could not suggest a free IP");
            return null;
        }
        ...
        if (dhcpStore.assignIP(HostId.hostId(mac), ipAssignment)) {
            log.info("allocated ip address is " + ipOffered.toString());
            return ipOffered;
        }

        log.info("could not allocate ip address");
        return null;
    }

    @Override
    public MacAddress getMacAddress(Ip4Address ip_address){
        return ip4_to_mac.get(ip_address);
    }
    ...
    public PortNumber getPortOnSwitch(MacAddress hostMac, DeviceId switchId){
        ...
        if (L == null){
            ...
            for (Pair<DeviceId, PortNumber> P:L){
                if (P.getKey().equals(switchId)){
                    return P.getValue();
                }
            log.info(String.format("No port for MAC %s found on Switch",
                    hostMac.toString(), switchId.toString()));
            }
        }
        return null;
    }
    ...
    private class DhcpPacketProcessor implements PacketProcessor {
        ...
        private void processDhcpPacket(PacketContext context, DHCP dhcpPayload) {
            ...
            PortNumber clientInPort = cp.port();
...
//                    log.info(String.format("RECVD DHCP DISCOVER FROM %s" +
//                            " FROM SWITCH %s", clientMac, switchId));
                    if (!knownLocation.containsKey(clientMac))
                        knownLocation.put(clientMac, new LinkedList<Pair<DeviceId, PortNumber>>());
                    knownLocation.get(clientMac).add(new Pair<DeviceId, PortNumber>(switchId,clientInPort));
                ...
                case DHCPREQUEST:
                    ...
                    if (dhcpStore.assignIP(hostId, ipAssignment)) {
                         ...
                         */
                        ip4_to_mac.putIfAbsent(requestedIP,clientMac);
//                        log.info(String.format("RECVD DHCP REQ FROM %s" +
//                                " FROM SWITCH %s", clientMac, switchId));
                    } else {
